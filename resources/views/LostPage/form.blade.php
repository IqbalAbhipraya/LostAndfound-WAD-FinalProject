...
<div class="max-w-md mx-auto bg-white rounded-lg shadow-md p-6 m-10">
        ...
        @if(isset($lostItem) && Auth::check() && (Auth::user()->id == $lostItem->userid || Auth::user()->id == $lostItem->lostid))
        <!-- Delete Button Form -->
        <form action="{{ route('lost-items.destroy', $lostItem->id) }}" method="POST" class="mt-4">
            @csrf
            @method('DELETE')
            <button type="submit"
                onclick="return confirm('Are you sure you want to delete this lost item? This action cannot be undone.');"
                class="w-full bg-red-600 hover:bg-red-700 text-white font-bold py-3 px-4 rounded-md transition-all duration-200 focus:outline-none focus:ring-2 focus:ring-red-500 focus:ring-offset-2 transform hover:scale-105 shadow-lg hover:shadow-xl flex items-center justify-center gap-2">
